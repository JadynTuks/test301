...
// Helper function to make commands testable
function createCommand(command, description, action) {
  ...
  // Store the action handler for testing
  cmd._actionHandler = action;
  return cmd;
}

// Database commands
createCommand('db:list', 'List all databases', async () => {
  try {
...
createCommand('db:create', 'Create a new database', async (dbName) => {
  try {
...
// Collection commands
createCommand('collection:list', 'List all collections in a database', async (dbName) => {
  try {
...
createCommand('collection:create', 'Create a new collection', async (dbName, collectionName) => {
  try {
...
// Document commands
createCommand('doc:list', 'List all documents in a collection', async (dbName, collectionName) => {
  try {
...
createCommand('doc:get', 'Get a document by ID', async (dbName, collectionName, documentId) => {
  try {
...
createCommand('doc:create', 'Create a new document', async (dbName, collectionName, options) => {
  try {
    let document;
...
createCommand('doc:update', 'Update a document', async (dbName, collectionName, documentId, options) => {
  try {
    let updates;
...
createCommand('doc:delete', 'Delete a document', async (dbName, collectionName, documentId) => {
  try {
...
createCommand('doc:query', 'Query documents in a collection', async (dbName, collectionName, options) => {
  try {
    let query;
...
createCommand('connect:test', 'Run comprehensive connectivity tests between CLI, API, and daemon', async () => {
  try {
...
createCommand('connect:repair', 'Attempt to automatically fix connection issues', async () => {
  try {
    console.log(chalk.blue('🔧 Running MPDB Connection Repair'));
    
    // Step 1: Clear the existing configuration
    const configPath = path.join(process.cwd(), 'db.json'); // Fixed path
...
createCommand('connect:config', 'Manually configure API endpoint', async () => {
  try {
